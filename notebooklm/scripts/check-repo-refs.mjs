...
// This guard must contain the forbidden strings it searches for, so exclude itself.
const SKIP_FILES = new Set([
  'notebooklm/scripts/check-repo-refs.mjs'
]);
...
async function walk(dir, files = []) {
  for (const entry of await readdir(dir, { withFileTypes: true })) {
    ...
    if (TEXT_EXTENSIONS.has(extname(entry.name)) || entry.name === 'LICENSE') files.push(join(dir, entry.name));
  }
  return files;
}
...
for (const file of await walk(repoRoot)) {
  ...
  if (ALLOW_HISTORICAL.has(rel) || SKIP_FILES.has(rel)) continue;

...
const PAGE_SIZE = 12;
const COLLECTIONS = [
  {id:'all', zh:'全部靈感', en:'All inspiration'},
  {id:'new', zh:'全新主題', en:'New themes'},
  {id:'dreamy', zh:'夢幻', en:'Dreamy'},
  {id:'animated', zh:'動畫感', en:'Animated'},
  {id:'adventure', zh:'冒險', en:'Adventure'},
  {id:'kids-favorite', zh:'孩童吸睛', en:'Kids’ favorites'},
  {id:'adult-vivid', zh:'35+ 生動', en:'Vivid 35+'},
  {id:'real-output', zh:'實際輸出', en:'Real output'}
];
const VIBE_LABELS = {
  dreamy:{zh:'夢幻',en:'Dreamy'}, animated:{zh:'動畫',en:'Animated'}, adventure:{zh:'冒險',en:'Adventure'},
  playful:{zh:'活潑',en:'Playful'}, cinematic:{zh:'電影感',en:'Cinematic'}, elegant:{zh:'優雅',en:'Elegant'},
  editorial:{zh:'編輯設計',en:'Editorial'}, calm:{zh:'舒展',en:'Calm'}, performance:{zh:'舞台',en:'Performance'}, cultural:{zh:'文化',en:'Cultural'}
};
...
const i18n = {
  'zh-TW': {
    ...
    tabLibrary: 'Prompt Library', tabLab: 'Prompt Lab', tabChain: 'Chain Builder', tabLint: '結構檢查', search: '搜尋', favorites: '☆ 收藏', export: '匯出', import: '匯入', age: '年齡', category: '分類', clearFilters: '清除篩選', emptyTitle: '找不到符合條件的 Prompt', emptyBody: '換一個關鍵字，或清除篩選條件。',
  ...
  en: {
    ...
    heroBody: 'Start with 132 age-adaptive prompts across 44 visual themes, including dreamy, animated, adventurous, cultural, and vivid mature directions. Everything runs locally in your browser.',
    ...
    tabLibrary: 'Prompt Library', tabLab: 'Prompt Lab', tabChain: 'Chain Builder', tabLint: 'Structure Check', search: 'Search', favorites: '☆ Favorites', export: 'Export', import: 'Import', age: 'Age', category: 'Category', clearFilters: 'Clear filters', emptyTitle: 'No matching prompts', emptyBody: 'Try another keyword or clear your filters.',
...
function safePreviewPath(value) {
  ...
  return /^\.\/assets\/real-previews\/[a-z0-9-]+\.png$/.test(path) ? path : null;
}
function collectionLabel(id) {
  const item = COLLECTIONS.find(collection => collection.id === id);
  return state.lang === 'zh-TW' ? item?.zh : item?.en;
}
function vibeLabel(id) {
  const item = VIBE_LABELS[id];
  return state.lang === 'zh-TW' ? item?.zh : item?.en;
}
function resetVisibleLimit() { state.visibleLimit = PAGE_SIZE; }
function restoreSimpleFiltersFromUrl() {
  const params = new URL(location.href).searchParams;
  const age = params.get('age');
  const collection = params.get('collection');
  if (['children', 'youth', 'adult'].includes(age)) state.age = age;
  if (COLLECTIONS.some(item => item.id === collection)) state.collection = collection;
  if (state.collection === 'adult-vivid') state.age = 'adult';
  if (state.collection === 'kids-favorite' && state.age === 'adult') state.age = 'children';
}
...
function filteredPrompts() {
  ...
  return allPrompts().filter(p => {
    ...
    if (state.collection !== 'all' && !(p.collections || []).includes(state.collection)) return false;
    if (state.favoritesOnly && !state.favorites.has(p.id)) return false;
    if (!q) return true;
    const hay = [localized(p.title), localized(p.summary), localized(p.category), p.prompt, p.themeId, p.ageGroup, p.vibe, p.energy, ...(p.collections || [])].join(' ').toLocaleLowerCase();
...
function renderFilters() {
  ...
  $('#collectionFilters').innerHTML = COLLECTIONS.map(collection => `<button class="chip collection-chip ${state.collection===collection.id?'active':''}" type="button" data-collection="${collection.id}">${escapeHtml(collectionLabel(collection.id))}</button>`).join('');
}

function renderLibrary() {
  const items = filteredPrompts();
  const visibleItems = items.slice(0, state.visibleLimit);
  $('#resultCount').textContent = state.lang === 'zh-TW'
    ? `${t('showing')} ${visibleItems.length} ${t('of')} ${items.length} ${t('results')}`
    : `${t('showing')} ${visibleItems.length} ${t('of')} ${items.length} ${t('results')}`;
  ...
  grid.innerHTML = visibleItems.map(p => {
    ...
    const previewPath = safePreviewPath(p.preview?.src);
...
function openPrompt(id, show = true) {
  ...
  $('#dialogBadges').innerHTML = `<span class="badge age">${escapeHtml(localized(p.ageLabel) || p.ageGroup)}</span><span class="badge vibe">${escapeHtml(vibeLabel(p.vibe) || localized(p.category))}</span><span class="badge score-good" title="${lint.score}/100">✓ ${t('ruleComplete')}</span>${(p.collections || []).includes('new')?`<span class="badge badge-new">${t('newTheme')}</span>`:''}`;
...
function renderWorkflowOptions() {
  ...
  $('#audienceSelect').innerHTML = [
    ...
    ['youth', state.lang==='zh-TW'?'青少年（16–34 歲）':'Youth (16–34)'],
...
function composePrompt() {
  ...
  const depthGuidance = {
    beginner: 'Depth setting: beginner-friendly. Define essential terms, use short explanations, and favor one clear idea at a time.',
    balanced: 'Depth setting: balanced. Explain the main ideas, relationships, and practical implications without unnecessary detail.',
    advanced: 'Depth setting: advanced. Preserve nuance, technical vocabulary, competing interpretations, and source-supported evidence while maintaining readable slides.'
  }[difficulty] || '';
  let body = '';
  if (workflowId === 'slide-deck') {
    ...
    body = base?.prompt || '';
    body = `Role and delivery lens: Act as a ${role}. Use this role only to improve explanation and organization; never let it override the uploaded sources.\n\nPrimary focus for this generation: ${focus}.\n\n${depthGuidance}\n\n${body}`;
  } else {
    const wf = state.workflows.find(w=>w.id===workflowId);
    const audienceLabel = audience==='children'?'learners aged 15 and below':audience==='youth'?'ages 16 to 34':'ages 35 and above';
    body = (wf?.starter || '')
...
function renderMiniLint(lint) {
  ...
  $('#labLintMini').innerHTML = missing.length ? missing.map(c=>`<div>• ${escapeHtml(state.lang==='zh-TW'?c.tipZh:c.tipEn)}</div>`).join('') : `<div>✓ ${state.lang==='zh-TW'?'核心規則完整；仍請檢查實際輸出。':'Core rules are complete; still review the generated output.'}</div>`;
}
...
function renderLint(lint) {
  ...
  $('#lintSummary').textContent = lint.score>=85 ? (state.lang==='zh-TW'?'規則結構完整；這不代表成品品質已驗證。':'Rule structure is complete; this does not verify output quality.') : lint.score>=65 ? (state.lang==='zh-TW'?'核心結構基本完整，但仍有可補強的規則。':'The core structure is present, with rules still worth improving.') : (state.lang==='zh-TW'?'建議先補齊核心規則再使用。':'Add the missing core rules before using it.');
...
async function importFile(file) {
  try {
    ...
    const normalized=incoming.filter(p=>p.prompt && (p.title||p.name)).map((p,i)=>({
      id:p.id||`custom-${Date.now()}-${i}`, themeId:p.themeId||`custom-${i}`, ageGroup:p.ageGroup||'youth', ageLabel:p.ageLabel||{'zh-TW':'自訂','en':'Custom'}, title:typeof p.title==='string'?{'zh-TW':p.title,'en':p.title}:p.title, summary:typeof p.summary==='string'?{'zh-TW':p.summary,'en':p.summary}:(p.summary||{'zh-TW':'自訂 Prompt','en':'Custom prompt'}), category:typeof p.category==='string'?{'zh-TW':p.category,'en':p.category}:(p.category||{'zh-TW':'自訂','en':'Custom'}), vibe:p.vibe||'editorial', energy:p.energy||'balanced', collections:Array.isArray(p.collections)?p.collections:[], prompt:p.prompt, sources:p.sources||[], custom:true
    }));
...
function bindEvents() {
  ...
  $('#searchInput').addEventListener('input',e=>{state.search=e.target.value;resetVisibleLimit();renderLibrary();});
  $('#ageFilters').addEventListener('click',e=>{const b=e.target.closest('[data-age]');if(!b)return;state.age=b.dataset.age;resetVisibleLimit();renderFilters();renderLibrary();});
  $('#categoryFilters').addEventListener('click',e=>{const b=e.target.closest('[data-category]');if(!b)return;state.category=b.dataset.category;resetVisibleLimit();renderFilters();renderLibrary();});
  $('#collectionFilters').addEventListener('click',e=>{const b=e.target.closest('[data-collection]');if(!b)return;state.collection=b.dataset.collection;if(state.collection==='adult-vivid')state.age='adult';if(state.collection==='kids-favorite'&&state.age==='adult')state.age='children';resetVisibleLimit();renderFilters();renderLibrary();});
...
async function init() {
  ...
  state.prompts = pData.prompts; state.workflows = wData.workflows; $('#statPrompts').textContent = pData.counts.prompts; $('#statThemes').textContent = pData.counts.themes;
  restoreSimpleFiltersFromUrl();
